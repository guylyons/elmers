...
let checks: [(String, () throws -> Void)] = [
    ...
    ("store rejects an empty database file untouched", store.testEmptyDatabaseFileIsRejectedAndLeftUntouched),
    ("stale journal files beside a missing database are set aside", store.testStaleJournalFilesBesideAMissingDatabaseAreSetAside),
    ("history folder is made private on every open", store.testHistoryFolderIsMadePrivateOnEveryOpen),
    ("read-only open of a missing database says the file is missing", store.testReadOnlyOpenOfAMissingDatabaseSaysTheFileIsMissing),
    ...
    ("quick paste numbers follow the modifier", interaction.testQuickPasteNumbersFollowTheModifier),
    ...
    ("search matches text case-insensitively and filters by type", history.testSearchMatchesTextCaseInsensitivelyAndFiltersType),
    ("search matches word starts and type names, not apps, as Paste does", history.testSearchMatchesWordStartsAndTypeNamesAsPaste),
