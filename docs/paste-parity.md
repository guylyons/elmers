...
| Card typography | Measured: title 15 pt semibold, time 12 pt, body 13 pt, footer 12 pt gray. | Larger than Paste in `CardView.swift` today (title 17 semibold, time 13, body 15, footer 13) after the September 16 request for bigger text. **Superseded September 17: the user asked to match Paste's sizes**, so dial these back to 15/12/13/12 and re-check the geometry tuned around them (tracked in `issues.md`). |
