...
- **Verified conversion and recovery.** First conversion builds a staging database, reloads it, compares every item and pinboard, then retains the original as `history.plist.migrated`. Real validation found an already-converted four-item database alongside a two-item plist written later by an older build. The recovery amendment backs up the plist and SQLite triplet, conservatively unions both histories, verifies a staged database, and archives the later plist as `history.plist.recovered`. The real result was 6 items and 2 pinboards; no item existed under the same UUID in both stores. The prior one-item migrated plist and pre-SQLite backup remained unchanged. A non-overwriting `history.plist.post-sqlite` now contains the fully merged history for rollback to an older build.
- **Evidence.** Before recovery: `history.plist` = 2 items / 0 pinboards / identity `3a4c13723b81`; `history.sqlite` = 4 items / 2 pinboards / identity `5377c32da733`; existing migrated plist = 1 item / 2 pinboards / identity `5314461d5c5e`. After recovery: SQLite = 6 items / 2 pinboards / identity `d95d7778ed63`. The synthetic live-capture check passed and produced a 7-item identity `da39adb1fbc1`; quitting and relaunching retained that exact count and identity, with no `history.plist` reappearing. The recovery directory is mode 0700 and contains mode-0600 copies of the pre-merge plist, database, WAL and SHM; active files and all retained archives are mode 0600.
...
- **Verification.** `scripts/test.sh` passes 36 checks with zero failures, including incremental writes, exact conversion, divergent-store recovery, corrupt reappeared-plist protection, private backups and non-overwriting rollback export. The rebuilt app passed all synthetic interaction, status-item, screenshot, sound and image-scroll suites; the latter measured 183 steps, p95 9.45 ms, max 15.79 ms, with no frame over 16.7 ms. The product change still emits the pre-existing non-Sendable closure warning in the interaction harness build.
- **Still open.** The real UI sequence for pinning, renaming and deleting harmless fixtures was not performed, to avoid operating on real card content without a private fixture-only view. Real capture plus restart persistence is verified. Stage 2 remains: lazy payload loading, FTS5 search and raising the 2,000-item limit. Paste Stack remains deferred; subscription/licensing stays excluded.
