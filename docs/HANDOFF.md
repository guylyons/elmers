...
- Later on September 16: typing a content-type word (any case) into the search filters by that type and shows a chip in the field (`SearchQuery.swift`). Core suite 19/19; `--check-interaction` 32 steps. Setting `ELMERS_CAPTURE_DIR` while running the interaction check writes a PNG of the demo panel, which is the safe way to eyeball the panel without exposing real history.
