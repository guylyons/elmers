...
Current verification: `scripts/test.sh` 39/39; bundled interaction, status-item, screenshot and sound checks pass; the 40-image scroll check measured p95 9.45 ms and max 15.79 ms. Storage regressions cover another writer remaining usable across recovery, consistent reads during concurrent saves, unique legacy retirement and fresh rollback exports. Full benchmark and identity evidence are in `docs/paste-parity.md`. The fixture-only live-storage check exercised create, pin, rename, delete, disk reload and cleanup through the real `AppModel`; real capture and full-process restart persistence are also verified.
