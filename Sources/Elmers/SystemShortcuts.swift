...
enum SystemShortcuts {
    static let usedMessage = String(localized: "This combination cannot be used because it is already used by a system-wide keyboard shortcut.\nIf you really want to use this key combination, most shortcuts can be changed in the Keyboard & Mouse panel in System Settings.")
    /// Paste's note for Option-only shortcuts, which the system cannot deliver reliably before macOS 15.2.
    static let optionMessage = String(localized: "Shortcuts using the Option key without Command or Control may not work correctly before macOS 15.2. Please consider updating to the latest version of macOS.")
